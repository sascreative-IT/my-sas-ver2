    ...
    <settings-layout>
      ...
      <span class="absolute inset-y-0 left-0 flex items-center pl-2">
          ...
          <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
               viewBox="0 0 24 24" class="w-6 h-6"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    ...
                    <input type="search" v-model="query"
                           ...
                           placeholder="Search..." autocomplete="off" @input="searchMaterials">
                </div>
            </div>
            <div class="mt-5">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr v-for="(material,index) in materials.data">
                        ...
                        <td class="px-2 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ material.marked_as_not_in_use }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <template v-if="!material.marked_as_not_in_use">
                                <button class="ml-2 text-red-600" @click="notInUseConfirmation(material)">
                                    Not In Use
                                </button>
                            </template>
        ...
        <delete-confirmation-modal
            ...
            :message='`Are you sure you want to mark as not in use "${this.selectedMaterial.name}" material ?`'
            :delete-url="'/settings/material/' + this.selectedMaterial.id"
            deleteButtonText="Yes"
            :show="confirmingNotInUseConfirmation"
            @close="confirmingNotInUseConfirmation = false"
        ></delete-confirmation-modal>
    </settings-layout>
</template>
...
export default {
    ...
    data() {
        return {
            query: '',
            selectedMaterial: {},
            confirmingNotInUseConfirmation: false
        }
    },
    mounted() {
        this.query = this.q;
    ...
    methods: {
        notInUseConfirmation(record) {
            this.selectedMaterial = record;
            this.confirmingNotInUseConfirmation = true;
        },
        searchMaterials() {
